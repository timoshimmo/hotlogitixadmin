...
const Assign = props => {
  ...
  useEffect(() => {
    ...
    (error) => {
      console.log("Error retrieving rider to assign:", error);
    });
  }, []);
 ...
 const handleChange = (event) => {
   if(event.target.value !== "") {
     ...
     DB.collection("riders").doc(event.target.value).get().then((doc) => {
       if (doc.exists) {
 ...
 const handleAssign = () => {
  ...
  .catch((error) => {
    setLoading(false);
    setServerError("Error assigning rider to trip: " + error);
    setFailed(true);
  });
 }
