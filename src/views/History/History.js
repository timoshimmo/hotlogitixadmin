...
const History = () => {
  ...
  useEffect(() => {
    ...
    const handleTripsList = () => {
      if(!loading) {
        ...
        .catch((error) => {
          setLoading(false);
          setServerError("Error retrieving trips list", error);
          console.log(error);
        })
  ...
  const handleFilter = query => {
    let newList = [];
    console.log(query);
    newList = query ? historyList.filter(history => String(history.from).includes(query)) : historyList;
    console.log(newList);
  ...
  return (
    <div className={classes.root}>
      ...
      <div className={classes.body}>
          ...
          <HistoryTable historyList={filteredHistory} handleFilter={handleFilter} />
      </div>
